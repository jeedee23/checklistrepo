...
import { loadConfig } from './data-persistence.js';
...
async function buildNewFieldDialog(dialog, fieldTypes) {
  ...
  const content = `
    ...
    <div class="modal-body">
      ...
      <div class="form-group mutually-exclusive" id="source-group" style="display: none;">
      ...
      <div class="form-group mutually-exclusive" id="options-group" style="display: none;">
        ...
        <small style="color: #666; font-size: 0.8em;">Use Options OR Source above, not both</small>
      </div>
      <div class="form-group">
...
function setupNewFieldEventHandlers(dialog) {
  console.log('[New Field Dialog] Setting up event handlers');
  ...
  console.log('[New Field Dialog] Found elements:', {
    createBtn: !!createBtn,
    fieldTypeSelect: !!fieldTypeSelect,
    sourceGroup: !!sourceGroup,
    optionsGroup: !!optionsGroup,
    fieldSourceSelect: !!fieldSourceSelect,
    fieldOptionsTextarea: !!fieldOptionsTextarea
  });
  
  // Show/hide source and options fields based on type selection
  if (fieldTypeSelect && sourceGroup && optionsGroup) {
    console.log('[New Field Dialog] Setting up field type change handler');
    fieldTypeSelect.addEventListener('change', () => {
      console.log('[New Field Dialog] Field type changed to:', fieldTypeSelect.value);
      if (fieldTypeSelect.value === 'select') {
        console.log('[New Field Dialog] Showing source and options groups');
        sourceGroup.style.display = 'block';
        optionsGroup.style.display = 'block';
      } else {
        console.log('[New Field Dialog] Hiding source and options groups');
        sourceGroup.style.display = 'none';
        optionsGroup.style.display = 'none';
        if (fieldSourceSelect) fieldSourceSelect.value = '';
        if (fieldOptionsTextarea) fieldOptionsTextarea.value = '';
      }
    });
  } else {
    console.log('[New Field Dialog] Missing elements:', { fieldTypeSelect, sourceGroup, optionsGroup });
  }
  
  // Mutual exclusivity: clear options when source is selected
  if (fieldSourceSelect && fieldOptionsTextarea) {
    fieldSourceSelect.addEventListener('change', () => {
      if (fieldSourceSelect.value) {
        fieldOptionsTextarea.value = '';
        fieldOptionsTextarea.placeholder = 'Options cleared - using Source instead';
      } else {
        fieldOptionsTextarea.placeholder = 'Enter options, one per line\nExample:\nLow\nNormal\nHigh';
      }
    });
  }
  
  // Mutual exclusivity: clear source when options are entered
  if (fieldOptionsTextarea && fieldSourceSelect) {
    fieldOptionsTextarea.addEventListener('input', () => {
      if (fieldOptionsTextarea.value.trim()) {
  ...
  if (createBtn) {
    createBtn.addEventListener('click', () => {
      ...
      // Build options object - source and options are mutually exclusive
      const options = {};
      
      // For select fields, validate mutual exclusivity
      if (fieldType === 'select') {
        const hasSource = fieldSource && fieldSource.trim();
        const hasOptions = fieldOptions && fieldOptions.trim();
        
        if (hasSource && hasOptions) {
          showNotification('error', 'Select fields cannot have both Source and Options. Please choose one.');
          return;
        }
        
        if (!hasSource && !hasOptions) {
          showNotification('error', 'Select fields must have either a Source or Options defined.');
          return;
        }
...
async function createNewField(fieldName, fieldType, fieldLabel, options = {}) {
  ...
  try {
    ...
    // Refresh the fields dialog to show the new field
    refreshFieldsDialog();
    
    // Close the new field dialog
    dialog.style.display = 'none';
    // refreshFieldsDialog();
...
/**
 * Refresh the fields dialog to show updated field list
 */
function refreshFieldsDialog() {
  const dialog = document.getElementById('fields-dialog');
  if (dialog && dialog.style.display !== 'none') {
    console.log('[Fields Dialog] Refreshing fields dialog');
    showFieldsDialog(); // This will rebuild the dialog with updated data
  }
}
...
// Listen for data updates to refresh UI components
document.addEventListener('dataUpdated', (event) => {
  if (event.detail.type === 'FIELDS') {
    console.log('[Fields Dialog] Field data updated, refreshing UI');
    refreshFieldsDialog();
  }
});
