  ...
  const content = `
    ...
    <div class="modal-body">
      <div class="form-group">
        ...
        <input type="text" id="field-name" placeholder="Enter field name" required>
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
        <label for="field-label">Display Label (optional):</label>
        <input type="text" id="field-label" placeholder="Leave empty to use field name">
      </div>
    </div>
    <div class="modal-footer">
...
function setupNewFieldEventHandlers(dialog) {
  ...
  if (createBtn) {
    createBtn.addEventListener('click', () => {
      ...
      createNewField(fieldName, fieldType, fieldLabel);
 ...
 */
async function fieldAlreadyExists(fieldName) {
  try {
    // Check in fields.json
    const response = await fetch('config/fields.json');
    const fieldsData = await response.json();
    const fields = fieldsData.fields || {};
    
    // Check if any field has the same key
...
/**
 * Create a new field (placeholder for now)
 */
async function createNewField(fieldName, fieldType, fieldLabel) {
  console.log('[New Field] Creating new field:', { fieldName, fieldType, fieldLabel });
  
  try {
    // TODO: Implement actual field creation
    // For now, just show a placeholder message
    showNotification('info', `Field creation is not yet implemented. Would create: ${fieldName} (${fieldType})`);
    
    // Placeholder: Add to fields.json
    // await updateFieldsJson(fieldName, fieldType, fieldLabel);
    
    // Placeholder: Refresh the fields dialog
    // refreshFieldsDialog();
    
  } catch (error) {
    console.error('Error creating new field:', error);
    showNotification('error', 'Failed to create new field');
  }
}
