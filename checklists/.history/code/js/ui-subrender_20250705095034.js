...
export function renderItem(item, tbody, path, columnDefs, rowHeight) {
  ...
  // IMPORTANT: Never modify item.hns here - use exactly as saved in file
        ...
        if (key==='label' || def.type==='tree') {
          ...
          if (Array.isArray(item.files)) {
            item.files.forEach(f => {
              ...
              // Check if it's a local file (not uploaded yet)
              if (!f.path.startsWith('checklists/')) {
                fileIcon.textContent = '📎⏳'; // Different icon for pending upload
                fileIcon.style.cursor = 'default';
                fileIcon.title = `${f.name} (will be uploaded when you save)`;
                fileIcon.style.opacity = '0.6'; // Make it slightly transparent
              } else {
          ...
          // New attachment system - files and links
          if (Array.isArray(item.attachments)) {
            item.attachments.forEach(attachment => {
              const attachIcon = document.createElement('span');
              attachIcon.style.cursor = 'pointer';
              attachIcon.style.marginRight = '2px';
              
              if (attachment.type === 'file') {
                // Use the SVG icon stored in the attachment, or fallback to emoji
                if (attachment.icon && attachment.icon.startsWith('<svg')) {
                  attachIcon.innerHTML = attachment.icon;
                } else {
                  attachIcon.textContent = attachment.icon || '📎';
                }
                attachIcon.title = `${attachment.name} (${(attachment.size / 1024).toFixed(1)}KB)`;
                attachIcon.onclick = e => { 
                  e.stopPropagation(); 
                  if (attachment.url) {
                    window.open(attachment.url, '_blank');
                  } else {
                    window.open(`files/${attachment.path}`, '_blank');
                  }
                };
              } else if (attachment.type === 'link') {
                // Use the SVG icon for links
                if (attachment.icon && attachment.icon.startsWith('<svg')) {
                  attachIcon.innerHTML = attachment.icon;
                } else {
                  attachIcon.textContent = '🔗';
                }
                attachIcon.title = attachment.text;
                attachIcon.onclick = e => { 
                  e.stopPropagation(); 
                  window.open(attachment.url, '_blank');
                };
              ...
              wrapper.appendChild(attachIcon);
            });
          }
          // Link icon & input
          if(item.link) {
