...
import { getUsersData, getCurrentUserIndex, setCurrentUserIndex, setCreateDialogCallback } from './user-management.js';

// Local functions for UserManagementDialog
function buildChecklistsDisplay(checklists) {
  ...
  return checklists.map(checklist => `
    <div class="checklist-item" onclick="openAccessManager('${checklist.id}')">
      <div><strong>${checklist.displayName}</strong></div>
      <div style="font-size: 8pt; color: #6c757d;">
        Access Level: ${checklist.accessLevel} | Layout: ${checklist.layoutName || 'Default'} | Last accessed: ${checklist.lastAccessed || 'Never'}
      </div>
    </div>
  `).join('');
}
...
function saveAndExit() {
  console.log('[User Management] Save and exit clicked');
  alert('Save and exit functionality not implemented yet');
}

function openAccessManager(checklistName) {
  console.log('[User Management] Opening access manager for:', checklistName);
  alert('Access manager functionality not implemented yet');
}
...
// Register this function as the dialog creation callback
setCreateDialogCallback(createUserManagementDialog);
