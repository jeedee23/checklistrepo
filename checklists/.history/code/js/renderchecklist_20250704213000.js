...
export async function renderChecklist(eventId = 0) {
  ...
  if (isStructuralChange) {
    ...
    if (sharedState.selectedPath) {
      ...
      if (itemAtPath) {
        ...
        console.log(`[renderChecklist] Selection after renumbering: path=${JSON.stringify(sharedState.selectedPath)}, item=${itemAtPath.label}, hns=${itemAtPathhns}`);
      } else {
        console.warn(`[renderChecklist] Could not find item at path after renumbering: ${JSON.stringify(sharedState.selectedPath)}`);
      }
    }
    
    console.log(`[renderChecklist] Renumbering complete: before=[path: ${JSON.stringify(beforePath)}, label: ${beforeLabel}], after=[path: ${JSON.stringify(sharedState.selectedPath)}, label: ${sharedState.selectedItem?.label}, no: ${sharedState.selectedItem?hns}]`);
  }
  ...
  if (isStructuralChange) {
    ...
    console.log('[renderChecklist] Current selected item before focus:', 
                ...
                `${sharedState.selectedItem.label} (${sharedState.selectedItemhns}) at path ${JSON.stringify(sharedState.selectedPath)}` :
                'None');
    ...
    if (sharedState.selectedItem && (!sharedState.selectedItem.hns || sharedState.selectedItem.hns === '')) {
      ...
      const itemAtPath = getItemByPath(sharedState.checklistData.items, sharedState.selectedPath);
      if (itemAtPath && itemAtPathhns) {
        sharedState.selectedItem = itemAtPath;
        console.log('[renderChecklist] Fixed selected item reference:', sharedState.selectedItemhns);
      }
    }
...
async function setFocusAfterStructuralChange() {
  ...
  console.log('[setFocusAfterStructuralChange] Attempting to focus item:', 
              ...
              'at path:', pathStr,
              'with number:', sharedState.selectedItemhns);
