...
async function saveChecklist() {
  ...
  try {
    ...
    traverse(checklistData.items, item => {
      if (item.files && Array.isArray(item.files)) {
        item.files.forEach(file => {
          // Check if file has local path (not already uploaded)
    ...
    if (filesToUpload.length > 0) {
      ...
      for (const { item, file } of filesToUpload) {
        try {
          // Generate a unique filename for the uploaded file
          const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
          const originalName = file.name || 'file';
          const uploadedFileName = `${timestamp}_${originalName}`;
          const uploadedPath = `checklists/files/${uploadedFileName}`;
          
          // Read the file from the local path
          const fileInput = document.querySelector(`input[type="file"][data-original-name="${file.name}"]`);
          if (fileInput && fileInput.files.length > 0) {
            ...
            const filePayload = {
              file: uploadedPath,
              content: base64,
              message: `Upload file: ${uploadedFileName}`
            };
            ...
            console.log(`✅ Uploaded file: ${file.name} → ${uploadedPath}`);
          }
        } catch (err) {
