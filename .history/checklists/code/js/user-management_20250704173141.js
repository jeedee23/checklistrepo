...
let userManagementTemplate = null; // Cache for the HTML template
...
async function loadTemplate(templatePath) {
  try {
    if (userManagementTemplate) {
      return userManagementTemplate; // Return cached version
    }
    ...
    userManagementTemplate = await response.text();
    console.log('[Template] Loaded user management template');
 ...
 */
async function createUserManagementDialog() {
  console.log('[User Management] Creating dialog, users data:', usersData);
  
  try {
    // Load the HTML template
    const template = await loadTemplate('templates/user-management-form.html');
    ...
    const templateVars = {
      current_user_display: `${currentUserIndex + 1}/${userCount}`,
      prev_disabled: currentUserIndex === 0 ? 'disabled' : '',
      next_disabled: currentUserIndex === userCount - 1 ? 'disabled' : '',
      current_index: currentUserIndex + 1,
      total_users: userCount,
      username: currentUser.username || '',
      user_id: currentUser.id || 'N/A',
      full_name: currentUser.fullName || '',
      created_date: currentUser.createdAt || 'N/A',
      email: currentUser.email || '',
      last_active: currentUser.lastActive || 'Never',
      token_display: currentUser.token ? `${currentUser.token.substr(0, 8)}...` : 'None',
      username_readonly: currentUser.username === 'admin' ? 'readonly' : '',
      access_level_options: generateAccessLevelOptions(currentUser.accessLevel || 5),
      user_status: currentUser.active !== false ? 'Active' : 'Inactive',
      two_factor_method: currentUser.twoFactorMethod || 'email',
      two_factor_status: generateTwoFactorStatus(currentUser),
      checklist_count: (currentUser.checklists || []).length,
      checklists_content: buildChecklistsDisplay(currentUser.checklists || []),
      delete_disabled: currentUser.username === 'admin' ? 'disabled' : ''
    };
    ...
    // Set the processed template as innerHTML
    dialog.innerHTML = processedTemplate;
