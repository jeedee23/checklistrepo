...
export function addFile() {
  ...
  fileInput.onchange = async function(event) {
    ...
    try {
      ...
      // Store file reference with local indicator
      sharedState.selectedItem.files.push({
        name: file.name,
        path: file.name, // Local path indicator (doesn't start with 'checklists/')
      ...
      // Store the actual file temporarily in a separate map (not in checklist data)
      if (!window.pendingFileUploads) {
        window.pendingFileUploads = new Map();
      }
      window.pendingFileUploads.set(file.name, file);
      ...
      // Show notification
      showNotification('success', `File "${file.name}" added to checklist. Will be uploaded when you save.`);
