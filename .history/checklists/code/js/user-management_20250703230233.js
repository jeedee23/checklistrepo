...
function buildUserManagementDialog(dialog) {
  ...
  dialog.innerHTML = `
    ...
    <div class="modal-body" style="padding: 20px; max-height: 70vh; overflow-y: auto;">
      <form id="user-form">
        <!-- User Navigation -->
        <div class="user-navigation" style="margin-bottom: 20px; padding: 10px; background: #f8f9fa; border-radius: 4px;">
          <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-weight: bold;">${currentUser.fullName} (${currentUser.username})</span>
            <div>
        ...
        <div class="form-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
          ...
          <div class="form-group">
            ...
            <input type="text" id="user-id" value="${currentUser.id}" readonly style="background: #f8f9fa;">
          </div>
          ...
          <div class="form-group">
            ...
            <input type="text" id="user-jobfunction" value="${currentUser.jobFunction}">
          </div>
          
          <div class="form-group">
            <label for="user-company">Company:</label>
            <input type="text" id="user-company" value="${currentUser.company}">
          </div>
          
          <div class="form-group">
            <label for="user-access-level">Default Access Level:</label>
            <select id="user-access-level">
              <option value="0" ${currentUser.defaultAccessLevel === 0 ? 'selected' : ''}>0 - Full Access</option>
              <option value="1" ${currentUser.defaultAccessLevel === 1 ? 'selected' : ''}>1 - Edit Access</option>
              <option value="2" ${currentUser.defaultAccessLevel === 2 ? 'selected' : ''}>2 - View Access</option>
              <option value="3" ${currentUser.defaultAccessLevel === 3 ? 'selected' : ''}>3 - Restricted Access</option>
            </select>
          </div>
          
          <div class="form-group">
            <label for="user-2fa-method">2FA Method:</label>
            <select id="user-2fa-method">
        ...
        <div class="form-section" style="margin-top: 20px;">
          <h4 style="margin-bottom: 10px; color: #495057;">Company Address</h4>
          <div class="form-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
            <div class="form-group">
              <label for="user-street">Street:</label>
              <input type="text" id="user-street" value="${currentUser.companyAddress?.street || ''}">
            </div>
            
            <div class="form-group">
              <label for="user-postal">Postal Code:</label>
              <input type="text" id="user-postal" value="${currentUser.companyAddress?.postalCode || ''}">
            </div>
            
            <div class="form-group">
              <label for="user-city">City:</label>
              <input type="text" id="user-city" value="${currentUser.companyAddress?.city || ''}">
            </div>
            
            <div class="form-group">
              <label for="user-country">Country:</label>
              <input type="text" id="user-country" value="${currentUser.companyAddress?.country || ''}">
            </div>
          </div>
        </div>
        ...
        <!-- User Checklists -->
        <div class="form-section" style="margin-top: 20px;">
          <h4 style="margin-bottom: 10px; color: #495057;">Assigned Checklists (${currentUser.checklists?.length || 0})</h4>
          <div id="user-checklists" style="max-height: 150px; overflow-y: auto; border: 1px solid #dee2e6; padding: 10px; background: #f8f9fa;">
        ...
        <!-- 2FA Status -->
        <div class="form-section" style="margin-top: 20px;">
          <h4 style="margin-bottom: 10px; color: #495057;">Two-Factor Authentication</h4>
          <div style="padding: 10px; background: #f8f9fa; border: 1px solid #dee2e6; border-radius: 4px;">
            <p>Current Method: <strong>${currentUser.twoFactorMethod || 'email'}</strong></p>
            <p>Status: <span class="status-badge ${currentUser.twoFactorMethod === 'totp' ? 'active' : 'inactive'}">
...
/**
 * Refresh the user dialog with current user data
 */
...
/**
 * Show notification (placeholder - you'll need to import this)
 */
function showNotification(type, message) {
  console.log(`[${type.toUpperCase()}] ${message}`);
  // TODO: Import and use actual notification system
}
