    ...
    switch (def.type) {
      ...
      default: {
        if (key==='label' || def.type==='tree') {
          ...
          // Attachment icons (new file upload system)
          if (Array.isArray(item.attachments)) {
            item.attachments.forEach(attachment => {
              const attachmentIcon = document.createElement('span');
              
              // Use inline SVG icon if available, otherwise fallback to emoji
              if (attachment.icon && attachment.icon.startsWith('data:image/svg+xml')) {
                const img = document.createElement('img');
                img.src = attachment.icon;
                img.style.width = '16px';
                img.style.height = '16px';
                img.style.cursor = 'pointer';
                img.title = `${attachment.fileName} (${(attachment.fileSize / 1024).toFixed(1)}KB)`;
                img.onclick = e => { 
                  e.stopPropagation(); 
                  // Open file in new tab
                  const fileURL = `${WORKER_URL}?file=${attachment.filePath}`;
                ...
                attachmentIcon.title = `${attachment.fileName} (${(attachment.fileSize / 1024).toFixed(1)}KB)`;
                ...
                wrapper.appendChild(attachmentIcon);
              }
            });
          }
          // Additional links (new link system)
