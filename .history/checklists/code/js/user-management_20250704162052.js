...
export function showUserManagementInContainer() {
  ...
  if (container) {
    // Create 200 lines of test text
    const testLines = Array.from({ length: 200 }, (_, i) => `Line ${i + 1}: This is a test`);
    ...
    container.innerHTML = `
      ...
      ">
        ${testLines.map(line => `<div>${line}</div>`).join('')}
      </div>
    `;
    
    // Override the CSS !important rule to prevent double scrollbars
