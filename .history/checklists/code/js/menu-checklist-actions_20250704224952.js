...
export function addFile() {
  ...
  fileInput.onchange = async function(event) {
    ...
    try {
      ...
      showNotification('info', 'Uploading file...');
      ...
      reader.onload = async function(e) {
        ...
        worker.onmessage = function(e) {
          if (e.data.success) {
            // Add file attachment to the item
            ...
            markSaveDirty('ADD_FILE');
            showNotification('success', `File "${file.name}" attached successfully`);
            
            // Re-render the item to show the new attachment
            renderChecklist();
          } else {
...
export function addLink() {
  ...
  modal.querySelector('#addLinkBtn').onclick = function() {
    ...
    markSaveDirty('ADD_FILE'); // Use same event type as files
